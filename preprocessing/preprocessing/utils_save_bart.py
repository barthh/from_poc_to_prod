...
class BaseTextCategorizationDataset:
    ...
    def _get_num_train_samples(self):
        ...
        """
        # TODO: CODE HERE
        nb_train_samples = self._get_num_samples() * self.train_ratio
        return(integer_floor(nb_train_samples))
    ...
    def _get_num_test_samples(self):
        ...
        (test set size)
        """
        nb_test_samples = self._get_num_samples() - self._get_num_train_samples()
        return(integer_floor(nb_test_samples))

    def _get_num_train_batches(self):
        """
        returns number of train batches
        """
        nb_train_batches = self._get_num_train_samples() / self.batch_size
        return(integer_floor(nb_train_batches))
    ...
    def _get_num_test_batches(self):
        ...
        """
        # TODO: CODE HERE
        nb_test_batches = self._get_num_test_samples() / self.batch_size
        return(integer_floor(nb_test_batches))
    ...
    def get_index_to_label_map(self):
        ...
        """
        # TODO: CODE HERE
        # return {index : label for index,label in enumerate(self._get_label_list())} #AA
        zipbObj = zip(self._get_label_list(), range(len(self._get_label_list())))
        return(dict(zipbObj))
    ...
    def get_label_to_index_map(self):
        ...
        """
        # TODO: CODE HERE
        # return {label: index for index, label in self.get_index_to_label_map().items()} #AA
...
class LocalTextCategorizationDataset(BaseTextCategorizationDataset):
    ...
    def __init__(self, filename, batch_size,
        ...
        # self._label_list = self._get_label_list() #AA
    ...
    def load_dataset(filename, min_samples_per_label):
        ...
        def filter_tags_with_less_than_x_samples(x):
            def filter_function(df):
                ...
                freq = df["tag_name"].value_counts()
                items = freq[freq >= x]
                return df[df['tag_name'].isin(items.index)]
        ...
        # TODO: CODE HERE
        df = df.pipe(filter_tag_position(0)).pipe(filter_tags_with_less_than_x_samples(min_samples_per_label))
        return(df)
    ...
    def _get_label_list(self):
        ...
        """
        # TODO: CODE HERE
        # return self._dataset['tag_name'].unique() #AA
    ...
    def get_train_batch(self):
        ...
        next_x = self.preprocess_text(self.x_train[i * self.batch_size : self.batch_size * (i + 1)])
        
        # TODO: CODE HERE
        # takes y_train between i * batch_size to (i + 1) * batch_size
        next_y = self.preprocess_text(self.y_train[i * self.batch_size : self.batch_size * (i + 1)])
    ...
    def get_test_batch(self):
        ...
        next_x = self.preprocess_text(self.x_test[i * self.batch_size : self.batch_size * (i + 1)])
        
        # TODO: CODE HERE
        # takes y_test between i * batch_size to (i + 1) * batch_size
        next_y = self.preprocess_text(self.y_test[i * self.batch_size : self.batch_size * (i + 1)])
